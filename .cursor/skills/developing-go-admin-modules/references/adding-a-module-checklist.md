...
按 [module-structure.md](module-structure.md) §2：`repo` 字段 unexported，`NewService` 里直接 `pkgrepo.NewRepository[T](processor)`。禁止为纯包装仓储建 `service/internal/repository/`。
...
func NewService(processor pkgrepo.ORMProcessor) *Service {
	return &Service{repo: pkgrepo.NewRepository[model.Notice](processor)}
}
...
// @route Post /notice
func (h *Handler) CreateNotice(c *context.Context, req *dto.CreateNoticeRequest) *context.Response {
...
func NewContainer(engine *xorm.Engine, hasher crypter.PasswordHasher, jwt *jwtauth.JWT) *Container {
	// ... 已有构造逻辑
	noticeSvc := noticeservice.NewService(processor) // 放在它依赖的对象之后构造
