...
默认：`NewService` 里直接 `pkgrepo.NewRepository[T](processor)`（见 [module-structure.md](module-structure.md) §2）：

```go
func NewService(processor pkgrepo.ORMProcessor) *Service {
	return &Service{repo: pkgrepo.NewRepository[model.SystemSetting](processor)}
}
```
...
_, err := repo.Transaction(ctx, func(txCtx context.Context) (any, error) {
	if err := repo.Create(txCtx, &user); err != nil {
		return nil, err // 返回 error 触发回滚
	}
	if err := otherRepo.Update(txCtx, &order); err != nil {
		return nil, err
	}
	return nil, nil // 返回 nil error 才会提交
})
```
...
- 方法名前缀（`Get`/`Post`/`Create`/`Update`/`Delete`/`Put`）决定 `AutoRouterRegister` 推断出的 HTTP method，必须与 `@route` 注释里声明的一致（注释本身目前不参与实际注册逻辑，是给人看的文档，保持同步避免误导）。
- 返回值统一用 `pkg/context` 包提供的构造函数：`context.Success`/`context.PageSuccess`/`context.NoContent`/`context.DatabaseError`/`context.BusinessError`/`context.InternalError`/`context.Unauthorized`，不要手写 `map[string]any` 拼响应体。
