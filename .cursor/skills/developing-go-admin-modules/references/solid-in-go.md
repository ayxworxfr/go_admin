...
**本项目的例外记录**：`pkg/repository.Repository[T]`/`ORMProcessor` 接口方法数偏多（十几个），这是刻意保留的例外，原因是它服务于通用 ORM 框架而非某个具体业务判断，见 go-oop-composition.md 的反例对照。**不要**以这个例外为理由，在写业务模块的跨模块接口时也放宽到十几个方法。
...
**本项目修复案例**：见 [module-structure.md](module-structure.md) §4（`Container` 替代全局 Instance）与 [SKILL.md](../SKILL.md) §3.3（跨模块窄接口）。唯一保留的全局状态例外是 `pkg/jwtauth.Instance`，仅限 `pkg/` 内部工具方法使用，业务模块禁止读取（见 module-structure.md §5）。
