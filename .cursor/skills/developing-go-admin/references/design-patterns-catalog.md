...
**本项目落点**：`internal/platform/router.Register`——优先查编译期路由表（`routes_gen.go`，由 `cmd/routegen` 从 `// @route` 生成），否则按方法名前缀推断 HTTP method 与路径。调用方（`SetupRoutes`）只管把 Handler 实例扔给 `RegisterStruct`：
...
新增 Handler 方法写好 `@route` 后执行 `make generate`（或 `go generate ./internal/platform/router/...`），不需要改动 `Register` 本身——这也是开闭原则的体现。Docker 构建会自动跑 routegen。
