...
│   ├── router/                        # Register（编译表 @route 优先，否则按方法名推断）
│   ├── routegen/                      # 构建期扫描 @route，生成 routes_gen.go
...
│   ├── routes.go                      # 构造 Handler → App.SetupRoutes
│   └── run.go                         # 应用启停编排
│
└── modules/                           # 按业务模块（限界上下文）划分
    ...
    │   ├── service/
    │   │   ├── user_service.go        # repo 字段直接 pkgrepo.NewRepository[model.User](db) 构造
...
| 场景 | 写法 | 范本 |
|---|---|---|
| 单实体、无自定义查询 | `Service` 内 unexported `repo`，`NewService` 里 `pkgrepo.NewRepository[T](db)` | `user`、`systemsetting` |
