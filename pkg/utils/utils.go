...
func GetAbsPath(path string) string {
	if projectPath, err := GetProjectRoot(); err == nil {
...
func GetProjectRoot() (string, error) {
	...
	// 假设项目根目录下有 go.mod 文件或conf目录
	for {
		if _, err := os.Stat(filepath.Join(wd, "go.mod")); err == nil {
			return wd, nil
		}
		if _, err := os.Stat(filepath.Join(wd, "conf")); err == nil {
			return wd, nil
		}
...
func ParseHTML(name, tmpl string, param any) template.HTML {
	t := template.New(name)
	t, err := t.Parse(tmpl)
	if err != nil {
...
func ParseText(name, tmpl string, param any) string {
	t := textTmpl.New(name)
	t, err := t.Parse(tmpl)
	if err != nil {
		fmt.Println("utils parseHTML error", err)
